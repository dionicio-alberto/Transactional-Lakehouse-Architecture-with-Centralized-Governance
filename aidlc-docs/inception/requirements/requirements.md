...
1. **Data Simulation**: Generate Users (synthetic PII) and Transactions datasets, joining with Kaggle Apple Stock Dataset (AAPL 1980-2026).
2. **Storage Strategy**: Data must traverse three Amazon S3 layers: `raw-zone`, `curated-zone`, and `athena-query-results`.
3. **ETL Processing**: Use AWS Glue job and PySpark to process raw datasets into Apache Iceberg format in the curated-zone with logical partitioning (e.g., year/month).
