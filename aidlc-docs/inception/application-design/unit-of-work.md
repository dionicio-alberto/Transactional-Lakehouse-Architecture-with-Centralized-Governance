...
- **Responsibilities**:
  ...
  - Produce raw CSV/Parquet files for the `raw-zone` in incremental batches.
...
- **Responsibilities**:
  - Transform raw data into Apache Iceberg format in the `curated-zone` incrementally.
  - Efficiently handle new data using Iceberg `MERGE INTO` or ACID appends.
